...
for (let i = 0; i < q9.length; i++) {
  arrayDog.push(q9[i].replace(/[.]/g, "").length);
}
...
for (let i = 0; i < q9.length; i++) {
  arrayMouse.push(q9[i].replace(/[.]/g, "").length);
}

const q13 = arrayMouse.reduce((a, b) => a + b, 0) / arrayMouse.length;
