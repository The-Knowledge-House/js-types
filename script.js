...
devotion to that cause for which they gave the last full measure of devotion—that we here highly resolve that these dead 
shall not have died in vain—that this nation, under God, shall have a new birth of freedom—and that government of the people, 
...
for (let i = 0; i < q9.length; i++) {
  noPeriods[i] = noPeriods[i].replace(".", "");
}
/**
 * QUESTION 12
...
for (let i = 0; i < q9.length; i++) {
  wordLength.push(q9[i].replace(/[.]/g, "").length);
}
/**
 * QUESTION 13
 ...
 */
// const q13 = null;
const q13 = averageLength;
let wordCount = [];
for (let i = 0; i < q12.length; i++) {
  wordCount += q12[i];
}
let averageLength = q13 / q12.length;
