...
const q4 = 2 + "Hello"; //isNaN(q6) will give true on the console
...
for (let i = 0; i < q9.length; i++) {
  arrayTwo.push(q9[i].replace(/[.]/g, "").length);
}
...
for (let i = 0; i < q9.length; i++) {
  arrayThree.push(q9[i].replace(/[.]/g, "").length);
}

const q13 = arrayThree.reduce((a, b) => a + b, 0) / arrayThree.length;
